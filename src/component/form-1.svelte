<script>
    ...
    //exporting variables from storybook to link the comp with storybook
    ...
    export let button_bg_color;
    export let firstName='';
    export let lastName= '';
    export let email='';
    export let phoneNumber= '';
    export let  messageArea= '' ;
  ...
  <div class="container">
    ...
    <h4>{formMessage}</h4>
      ...
      <input type="text" class="form-control {errorsValid.firstNameValid ? '' : 'invalid'}" bind:value="{firstName}" on:keyup="{validateFirstName}" placeholder="First name" required>
          </div>
  
          <div class="form-group">
      <input type="text" class="form-control {errorsValid.lastNameValid ? '' : 'invalid'}" bind:value="{lastName}" on:keyup="{validateLastName}"  placeholder="Last name" required>
      </div> 
      
      <div class="form-group">
              <input type="email" class="form-control {errorsValid.emailValid ? '' : 'invalid'}" bind:value="{email}" on:keyup="{validateEmailAddress}"  placeholder="Email" required>
      </div> 
      
      <div class="form-group">
              <input type="text" class="form-control {errorsValid.phoneNumberValid ? '' : 'invalid'}" bind:value="{phoneNumber}" on:keyup="{validatePhoneNumber}"  placeholder="Contact number" required>
      </div> 
      
      <div class="form-group">
        <textarea rows="4" class="form-control" bind:value="{messageArea}" placeholder="Give us the deets"></textarea>
          </div> 
  
  
          <button class="btn btn-full" style="background-color:{button_bg_color};color:{button_text_color}" on:click={() => submitted = true} >Continue</button>
      </form>
  </div>
